FactoryBot.define do
  factory :item do
    ...
    if product_name.length > product_name_max_length
      product_name = product_name[0, product_name_max_length]
    end

    paragraph_max_length = 1000
    paragraph = Faker::Lorem.paragraph
    if paragraph.length > paragraph_max_length
      paragraph = paragraph[0, paragraph_max_length]
    end
    ...
    item_price             {Faker::Commerce.price(range: 300..9999999, as_string: true).split('.').first.to_i}
